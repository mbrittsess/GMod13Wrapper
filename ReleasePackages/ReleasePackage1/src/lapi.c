...
LUA_API const char     *(lua_typename) (lua_State *L, int tp) {
    ...
    return ((tp <= LUA_TTHREAD) && (tp >= LUA_TNONE)) ? TypeNames[tp+1] : "invalid type";
};
...
LUA_API void	       *(lua_touserdata) (lua_State *L, int idx) {
    ...
    switch (IdxClassification(L, idx, NULL, NULL)) {
        ...
        case VanIdx_Pseudo_Upv :
            if (!CheckUpv(L, UpvalueIdxToOrdinal(L, idx))) {
                return NULL; /* Wait, should we error or return NULL? Does CheckUpv() raise errors? */
            } else {
                break;
            };
    };
    ...
    lua_pushvalue(L, idx);
    if (lua_isuserdata(L, -1)) { /* The procedure is now the same for both of them. (full and light, that is) */
...
this, we have to *make* Garry's Mod report "userdata".
     --- Following information not completely correct anymore, check following block comment for details ---
...
LUA_API int  (lua_yield) (lua_State *L, int nresults) {
    luaL_error(L, "In library '%s': Vanillin lacks the capability to yield its own thread!", OrigRequireName);
    return 0;
};
...
LUA_API int  (lua_resume) (lua_State *L, int narg) {
    luaL_error(L, "In library '%s': Vanillin lacks the capability to start threads!", OrigRequireName);
    return 0;
};
...
LUA_API int  (lua_status) (lua_State *L) {
    luaL_error(L, "In library '%s': Vanillin lacks the capability to report thread status!", OrigRequireName);
    return 0;
};
...
LUA_API lua_Alloc (lua_getallocf) (lua_State *L, void **ud) {
    luaL_error(L, "In library '%s': Vanillin lacks the capability to retrieve a state's memory-allocation function!", OrigRequireName);
    return NULL;
};
...
LUA_API void lua_setallocf (lua_State *L, lua_Alloc f, void *ud) {
    luaL_error(L, "In library '%s': Vanillin lacks the capability to set a state's memory-allocation function!", OrigRequireName);
    return;
};
